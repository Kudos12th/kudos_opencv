...
def bev(image):
    ...
    #280
    view=[[140, 330], [100, 480], [440, 330], [480, 480]] #좌상 좌하 우상 우하
...
def find_yellow(img) :
    # bgr
    ye_low_bgr = np.array([25, 152, 90])
    ye_upp_bgr = np.array([138, 255, 185])
    # ye_low_bgr = np.array([30, 90, 40])
    # ye_upp_bgr = np.array([150, 255, 180])
...
def huddle_detect(huddle_contours, huddle_hier, out_img):
    ...
    if sorted_centers:
        cv2.circle(out_img, sorted_centers[0], 5, (0, 0, 255), -1)
...
def cal_slope(contours,img) :
    ...
    if left_points:
        left_yellow = max(left_points, key=lambda point: point[1])
    if right_points:
        right_yellow = max(right_points, key=lambda point: point[1]) 
...
while True :
    ...
    if huddle_centers:
        if huddle_centers[0][1] >= 380:
            ...
            # print("hurdle stop")
            print('slope :', slope,', zero_cnt :',zero_cnt)
